...
(uv)^n=u^{(n)}v + nu^{(n-1)}v' + \frac{n(n-1)}{2!} u^{(n-2)}v'' + ... + \frac{n(n-1)...(n-k+1)}{k!}u^{(n-k)}v^{(k)} + ...+ uv^{(n)}
```
# ref:显函数与隐函数
## 显函数
```latex 
因变量y已经表示成自变量x的明显的数学表达式
```

## 隐函数
```latex
对x、y的二元方程F(x,y)=0，如果存在y=f(x)，s.t.\quad  F(x,f(x)) \equiv 0，则称f(x)是由F(x,y)=0所确定的隐函数\\
求隐函数的过程叫做隐函数的显化
```